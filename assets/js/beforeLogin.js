...
$(document).ready(function () {
			...
			submitHandler: function () {
				...
				$.ajax({
					...
					success: function (data) {
						...
						if (sUserType == "LENDER" && redirectUrl != "" && data.status != "INACTIVE") {
							window.location = redirectUrl;
						} else if (sUserType == "LENDER" && redirectUrl == "" && data.status != "INACTIVE") {
							window.location = "idb";
						}
						else if (sUserType == "LENDER" && redirectUrl == "" && data.status == "INACTIVE") {
							window.location = "activateuser";
						} else if (sUserType == "LENDER" && redirectUrl == "" && data.status == "INACTIVE") {
							window.location = "activateuser";
							...
							window.location = "admin/testdashboard";
						}
						else if (sUserType == "SUPERADMIN" && redirectUrl == "") {
							window.location = "admin/superdashboard";
						}
						else if (sUserType == "PRIMARYADMIN" && redirectUrl == "") {
							window.location = "admin/primarydashboard";
						}
						else if (sUserType == "STUDENTADMIN" && redirectUrl == "") {
							window.location = "admin/studentdashboard";
						}
						else if (sUserType == "BORROWERADMIN" && redirectUrl == "") {
							window.location = "admin/borrowerdashboard";
						}
						else if (sUserType == "MASTERADMIN" && redirectUrl == "") {
							window.location = "admin/masterdashboard";
						} else if (
							sUserType == "ADMIN" ||
							...
							window.location = "borrowerDashboa";
						}
					},
					statusCode: {
					...
					else if (sUserType == "TESTADMIN") {
						window.location = "test/dashboard";
					}
					else if (sUserType == "SUPERADMIN") {
						window.location = "super/dashboard";
					}
					else if (sUserType == "PRIMARYADMIN") {
						window.location = "primary/dashboard";
					}
					else if (sUserType == "STUDENTADMIN") {
						window.location = "student/dashboard";
					}
					else if (sUserType == "BORROWERADMIN") {
						window.location = "admin/borrowerdashboard";
					}
					else if (sUserType == "MASTERADMIN") {
						window.location = "admin/masterdashboard";
					} else {
	...
	$("#lendermobileverifybutton").click(function (event) {
		...
		if (mobileNumber.length == 10) {
			if (userisIn == "local") {
				var getStatUrl =
					"http://ec2-15-207-239-145.ap-south-1.compute.amazonaws.com:8080/oxyloans/v1/user/newregister";
			} else {
				var getStatUrl =
	...
	$("#getWhatsAppLogin").click(function (event) {
		// $("#getWhatsAppLogin").attr("disabled",true);
		...
		const mappeduserslect = $("#whatsappmappedUsersId").val();
		console.log(mappeduserslect);
		if (whatsappNo == "") {
		...
		if (userisIn == "local") {
			var WhatsAppLoginUrls =
				"http://ec2-15-207-239-145.ap-south-1.compute.amazonaws.com:8080/oxyloans/v1/user/whatsapp-login-otp";
		} else {
			var WhatsAppLoginUrls =
		...
		if (mappeduserslect == null) {
			var postData = { whatsappNumber: countrycode + whatsappNo };
		} else {
			var postData = {
				whatsappNumber: countrycode + whatsappNo,
				id: mappeduserslect.substring(2)
			};
		}
		...
		$.ajax({
			...
			success: function (data, textStatus, xhr) {
				$(".displayScreenFlowRefresh").hide();
				$("#getWhatsAppLogin").attr("disabled", false);
		...
		$.ajax({
			...
			success: function (data) {
				...
				if (data.message == null) {
					...
					sessionStorage.setItem(
						"whatsapploginUsers",
						JSON.stringify(data.whatsappLoginResponse)
					);
					const obj = JSON.parse(sessionStorage.getItem("whatsapploginUsers"));

					setTimeout(() => {
						window.location = "userSociallogin"
					}, 1000);
			...
			statusCode: {
				...
				400: function (response) {
					...
					$(".whatsapplogInOtperror").html(response.responseJSON.errorMessage);
					$(".whatsapplogInOtperror").show();
					// alert("1");
					bootbox.alert(
						'<span style="color:Red;">Error While Saving Outage Entry Please Check</span>',
	...
	$("#lenderwhatsappverifybutton").click(function (event) {
		...
		//var getStatUrl = "http://ec2-15-207-239-145.ap-south-1.compute.amazonaws.com:8080/oxyloans/v1/user/newregister";
		var postData = { whatsappNumber: whatsappnumber, id: userIDBwhatsapp };
		var postData = JSON.stringify(postData);
			...
			$.ajax({
				...
				success: function (data) {
					...
					if (data.emailVerified == true) {
						if (sUserType == "LENDER" && data.status != "INACTIVE") {
							window.location = "idb";
						} else if (sUserType == "LENDER" && data.status == "INACTIVE") {
							window.location = "activateuser";
						} else {
							window.location = "borrowerDashboard";
						}
	...
	$("#resetbtn").click(function () {
		...
		var enteredemailValue = $("#email").val();
		var postData = { email: enteredemailValue, projectType: "OXYLOANS" };
		var postData = JSON.stringify(postData);
		console.log(postData);
		...
		if (userisIn == "local") {
			var passwordregUrl =
				"http://ec2-15-207-239-145.ap-south-1.compute.amazonaws.com:8080/oxyloans/v1/user/resetpassword";
		} else {
			var passwordregUrl =
	...
	$(document).ready(function () {
		$("#sendActivationEmail").click(function () {
			...
			if (userisIn == "local") {
				var sendactivation =
					"http://ec2-15-207-239-145.ap-south-1.compute.amazonaws.com:8080/oxyloans/v1/user/emailIdUpdationToOldUsers";
			} else {
				var sendactivation =
...
function checkingUserInDb(email, googleUserId) {
	...
	if (userisIn == "local") {
		var updateCommentUrl =
			"http://ec2-15-207-239-145.ap-south-1.compute.amazonaws.com:8080/oxyloans/v1/user/veifyExistenceOfSocialLoginUser";
	} else {
		var updateCommentUrl =
...
function searchExperts() {
	...
	if (userisIn == "local") {
		var getExpert =
			"http://ec2-15-207-239-145.ap-south-1.compute.amazonaws.com:8080/oxyloans/v1/user/uniqueLendersSearchCall/" +
			suserId;
	} else {
		var getExpert =
...
function getlistOfUniqueLenders() {
	if (userisIn == "local") {
		var getUnique =
			"http://ec2-15-207-239-145.ap-south-1.compute.amazonaws.com:8080/oxyloans/v1/user/listOfUniqueLenders";
	} else {
		var getUnique =
...
function getexpertMobileOtp() {
	...
	if (userisIn == "local") {
		var getStatUrl =
			"http://ec2-15-207-239-145.ap-south-1.compute.amazonaws.com:8080/oxyloans/v1/user/sendMobileOtp";
	} else {
		var getStatUrl =
...
function getexpertEmailOtp() {
	...
	if (userisIn == "local") {
		var getStatUrl =
			"http://ec2-15-207-239-145.ap-south-1.compute.amazonaws.com:8080/oxyloans/v1/user/sendEmailOtp";
	} else {
		var getStatUrl =
...
$(document).ready(function () {
	...
	$("#userRegisterSection").validate({
		...
		messages: {
			...
			regMobileNumber: {
				...
				minlength: "Mobile number should be 10 degits.",
				maxlength: "Mobile number should not be greater than 10 degits.",
		...
		submitHandler: function () {
			...
			if (userisIn == "local") {
				var regURL_new =
					"http://ec2-15-207-239-145.ap-south-1.compute.amazonaws.com:8080/oxyloans/v1/user/newUserRegistration";
			} else {
				var regURL_new =
	...
	$(".sendActivationLink").click(function (event) {
		...
		if (userisIn == "local") {
			var sendActivationEmail =
				"http://ec2-15-207-239-145.ap-south-1.compute.amazonaws.com:8080/oxyloans/v1/user/sendingEmailActivationLink";
		} else {
			var sendActivationEmail =
	...
	$("#userRegisterSectionStep2").validate({
		...
		submitHandler: function () {
			...
			if (userisIn == "local") {
				var regURL_new =
					"http://ec2-15-207-239-145.ap-south-1.compute.amazonaws.com:8080/oxyloans/v1/user/emailVerification";
			} else {
				var regURL_new =
...
$(document).ready(function () {
	$("#partnerRegisterSection").validate({
		...
		submitHandler: function () {
			...
			if (userisIn == "local") {
				var regURL_Partner =
					"http://ec2-15-207-239-145.ap-south-1.compute.amazonaws.com:8080/oxyloans/v1/user/partnerRegistrationFlow";
			} else {
				var regURL_Partner =
...
const oxyFreeWhatsappbulk = () => {
	...
	const whatsappnumber =
		userisIn == "local"
			? "http://ec2-15-207-239-145.ap-south-1.compute.amazonaws.com:8080/oxyloans/v1/user/oxyWhatsappThroughExcel"
...
const oxyfreewhatsapps = () => {
	...
	if (userisIn == "local") {
		whatsAppNumber =
			"http://ec2-15-207-239-145.ap-south-1.compute.amazonaws.com:8080/oxyloans/v1/user/oxyWhatsapp";
	} else {
		whatsAppNumber =
...
const Otplessintegration = (data) => {
	...
	if (userisIn == "local") {
		var otpless =
			"http://ec2-15-207-239-145.ap-south-1.compute.amazonaws.com:8080/oxyloans/v1/user/otpless-login";
	} else {
